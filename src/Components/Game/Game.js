...
import { saveGameAndLog } from "../../helpers";
import { gameControl, loadGames, loadLog, loadRosters, loadPlayers, clearGame } from "../../AC";
import { FORCE_UPLOAD_GAME, TIME_PAUSE, TIME_START, TIME_STOP } from "../../constants";
...
const Game = ({ id, tournamentID, classes }) => {
    const [tabValue, setTabValue] = useState(0);
    ...
    const game = useSelector(state => state.games.list.get(id));
    const log = useSelector(state => state.logs.list.get(game.logID));

    const toggleTimer = () => {
        dispatch(gameControl(game.isTimeOn ? TIME_PAUSE : TIME_START, game));
    };
    ...
    const handlerStop = () => {
        ...
        if (window.confirm('Вы хотите завершить игру?') && !game.isFinished) {
            dispatch(gameControl(TIME_STOP, game));
        }
    };

    const handlerForceUploadGame = () => {
        dispatch(gameControl(FORCE_UPLOAD_GAME, game));
    };
